...
public class ProductModel {
	...
	public boolean deleteProduct(ProductBean beanObject) {
		 ...
		 * 
		 * select COUNT(ad.merchantlocationID) from ad_merchant ad, ad_product
		 * ap where ad.adID = ap.adID and ap.productID=2; //Select the
		 * advertisementIDs to delete from the merchantlocation table.
		 * 
		 * delete merchantlocation, ad_merchant from merchantlocation,
		 * ad_merchant where (merchantlocation.merchantlocationID =
		 * ad_merchant.merchantlocationID) and (ad_merchant.adID IN (select
		 * DISTINCT a.adID from ad_product ad, advertisement a, product p where
		 * p.productID = ad.productID and p.productID=2;));
		 * 
		 * Select the ads to be deleted select DISTINCT a.adID from ad_product
		 * ad, advertisement a, product p where p.productID = ad.productID and
		 * p.productID=2; delete advertisement, ad_product from advertisement,
		 * ad_product where (advertisement.adID=ad_product.adID) and
		 * (ad_product.productID=1);
		 * 
		 * Select the product to be deleted. select DISTINCT a.adID from
		 * ad_product ad, advertisement a, product p where p.productID =
		 * ad.productID and p.productID=1; Have to modify this method, take care
		 * of the cascading.
		 */
		...
		try {
			...
			String qry1 = "select DISTINCT m.merchantlocationID as mID from merchantlocation m, ad_merchant ad, ad_product ap where ap.adId=ad.adID and ap.productID ="
					+ beanObject.getCount() + ";";
			...
			if (empty) {
				...
				String dqlQry1 = "delete merchantlocation, ad_merchant from merchantlocation, ad_merchant where (merchantlocation.merchantlocationID = ad_merchant.merchantlocationID) and (ad_merchant.adID IN (select DISTINCT a.adID from ad_product ad, advertisement a, product p where p.productID = ad.productID and p.productID="
						+ beanObject.getCount() + "));";
				// String delQry =
				// "DELETE FROM merchantlocation WHERE merchantlocationID= "+rsMerchant.getInt("mID")+";";
			...
			String qry2 = "select DISTINCT ad.adID from ad_product ad, product p where p.productID="
					+ beanObject.getCount() + ";";
			...
			if (empty1) {
				...
				// Delete the records;
				rsRead.absolute(1);
				String delQry1 = "delete advertisement, ad_product from advertisement, ad_product where (advertisement.adID=ad_product.adID) and (ad_product.productID="
						+ beanObject.getCount() + ");";
